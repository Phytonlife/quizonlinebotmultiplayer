...
def create_room(message):
    ...
    rooms[room_id] = {"owner": message.from_user.username, "members": [message.from_user.username]}
...
def join_specific_room(message):
    ...
    if room_id in rooms:
        if message.from_user.username not in rooms[room_id]["members"]:
            rooms[room_id]["members"].append(message.from_user.username)
        members_list = "\n".join(rooms[room_id]["members"])
        bot.send_message(message.chat.id,
                         f"Вы присоединились к комнате {rooms[room_id]['owner']}.\n\nУчастники комнаты:\n{members_list}")
    else:
